...
function operacion(matriz,vector,long) {
	...
	$('.paso3').append("<h4>Paso 3:</h4>");
	$('.paso3').append("<h5>M Jacobi (M=-D^-1 * C):</h5>");
	mostrarM(mJ,'paso3',long);

	$('.paso4').append("<h4>Paso 4:</h4>");
	$('.paso4').append("<h5>N Jacobi (N=D^-1 * V):</h5>");
	mostrarV(mN,'paso4',long);

	$('.paso5').append("<h4>Iteracciones:</h4>");
	$('.paso5').append("<h5>Vector Inicial:</h5>");
	for(var i=0;i<long;i++) {
		$('.paso5').append("<div><input type='number' class='vectorX' value='0' readonly></div>");
	}
	...
	resX = iteractivaX(mJ,vecI,mN);
	for(var i=0;i<15;i++) {
		resX = iteractivaX(mJ,resX,mN);
	}
	$('.paso5').append("<h5>Vector Resultado Convergente:</h5>");
	mostrarV(resX,'paso5',long);
}
